...
Zano Sheets is a Microsoft Excel Add-in with an integrated AI chat panel. Users can chat with LLM providers (OpenAI, Anthropic, Google, etc.) directly within Excel using their own API keys (BYOK). The agent has Excel read/write tools, a sandboxed bash shell, and a virtual filesystem for file uploads.
