...
function buildRangeAddress(
  ...
  const endRow = row + rows; // row 0 + 10 rows = endRow 10 (A1:A10)
...
const webSearchCmd: Command = defineCommand("web-search", async (args) => {
  ...
  for (const arg of args) {
    ...
    if (match) {
      ...
      return {
        stdout: "",
        stderr: `Invalid flag: ${arg}. Supported flags: --max, --region, --time, --page, --json`,
        exitCode: 1,
      };
    } else {
      positional.push(arg);
  ...
  const maxResults = flags.max ? Number.parseInt(flags.max, 10) : 10;
  if (!Number.isInteger(maxResults) || maxResults < 1 || maxResults > 50) {
    return {
      stdout: "",
      stderr: "Invalid --max value. Must be an integer between 1 and 50.",
      exitCode: 1,
    };
  }

  const page = flags.page ? Number.parseInt(flags.page, 10) : undefined;
  if (page !== undefined && (!Number.isInteger(page) || page < 1)) {
    return {
      stdout: "",
      stderr: "Invalid --page value. Must be an integer >= 1.",
      exitCode: 1,
    };
  }

  const timelimit = flags.time as "d" | "w" | "m" | "y" | undefined;
  if (timelimit && !["d", "w", "m", "y"].includes(timelimit)) {
    return {
      stdout: "",
      stderr: "Invalid --time value. Use one of: d, w, m, y.",
      exitCode: 1,
    };
  }
  ...
  if (region && !/^[a-z]{2}-[a-z]{2}$/i.test(region)) {
    return {
      stdout: "",
      stderr: "Invalid --region value. Expected format like us-en or uk-en.",
      exitCode: 1,
    };
  }
